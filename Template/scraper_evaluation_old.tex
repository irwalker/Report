...
Because each tweet must be fetched with a separate HTTP request, the number of tweets on a profile is the primary factor in profile scrape time. Thus in my evaluation the primary performance metric used is the rate of Tweets scraped per second. 
...
Greater parallelization - over the ECS Grid for example, rather than from a  single machine. Parallelize components other than tweet fetching - entire profile fetching could be split up. Over engineered? 
